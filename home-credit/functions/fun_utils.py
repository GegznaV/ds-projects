...
def highlight_min(s, color="green"):
    """Helper function to highlight the minimum in a Series or DataFrame
    ...
    >>> import pandas as pd
    >>> data_frame = pd.DataFrame({'col1': [1, 2], 'col2': [3, 4]})
    >>> data_frame.style.apply(highlight_min)

    >>> data_frame.style.format(precision=2).apply(highlight_min)
    """
    is_min = s == s.min()
    return [f"color: {color}" if cell else "" for cell in is_min]
...
def highlight_int_float_text(value, color="deepskyblue"):
    if bool(re.search(r"([Ii]nt|[Ff]loat)", str(value))):
        return f"color: {color}"
    else:
        return ""


# Function to change text color for data types containing "date" or "time"
def highlight_date_or_time_text(value, color="violet"):
    if bool(re.search(r"([Dd]ate|[Tt]ime)", str(value))):
        return f"color: {color}"
    else:
        return ""


# Function to change text color for data types containing "bool"
def highlight_bool_text(value, color="darkorange4"):
    if bool(re.search(r"[Bb]ool", str(value))):
        return f"color: {color}"
    else:
        return ""
...
def highlight_below_above_str(
    ...
    if above < value_num:
        return f"color: {color_above}"
    elif value_num < below:
        return f"color: {color_below}"
    else:
        return ""
