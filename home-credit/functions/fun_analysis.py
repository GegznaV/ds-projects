...
def style_col_info(
    ...
    ***
    ...
    def fade_p_dom_excl_na_wo_na(x):
        c1 = f"color: {color_fade}"
        c2 = ""

        # Info is initial dataset
        mask = info["n_missing"] == 0

        df1 = pd.DataFrame(c2, index=x.index, columns=x.columns)
        # modify values of df1 column by boolean mask
        df1.loc[mask, "p_dom_excl_na"] = c1
        return df1

    def fade_p_dominant_with_na(x):
        c1 = f"color: {color_fade}"
        c2 = ""

        # Info is initial dataset
        mask = (info["p_dominant"] == info["p_dom_excl_na"]) & (info["n_missing"] > 0)

        df1 = pd.DataFrame(c2, index=x.index, columns=x.columns)
        # modify values of df1 column by boolean mask
        df1.loc[mask, "p_dominant"] = c1
        return df1
    ...
    return (
        ...
        .map(my.highlight_value, when=0, color=color_fade, subset=["p_dom_excl_na"])
        .apply(fade_p_dom_excl_na_wo_na, axis=None, subset=["p_dom_excl_na"])
        .apply(fade_p_dominant_with_na, axis=None, subset=["p_dominant"])
    )
