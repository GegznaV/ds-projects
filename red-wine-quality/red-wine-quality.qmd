...
**RStudio** is an IDE for R and Python. In Quarto (`.qmd`) and R Markdown (`.rmd`) documents, RStudio supports an interface that allows using R and Python in a single document (e.g., we can access Python objects in R code cells by using `py$<python_object_name>`{.r} accessor, and access R objects in Pthon code cells by using `r.<r_object_name>`{.py} accessor). So additional technical tasks for this project were to:
...
Main principles how it was lewveraged between R and Python in this project:
